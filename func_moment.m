%重心周りの地面反力と壁のモーメントの釣り合いを計算
%
function [moment_CoM] = func_moment(q,m_list,l_link_list,GRF,r,F)
    ...
    for i=1:size(q(:,1),1)
        %重心から踵までのベクトル
        vector1=[(coordinates_x(i,4)-COM_link(i,1)) (coordinates_y(i,4)-r-COM_link(i,2))];
        %踵の床反力のx方向成分の力
        ...
        %重心からつま先までのベクトル
        vector2=[(coordinates_x(i,5)-COM_link(i,1)) (coordinates_y(i,5)-COM_link(i,2))];
        %つま先の床反力のx方向成分の力
        ...
        %重心から股関節までのベクトル
        vector3=[(coordinates_x(i,2)-COM_link(i,1)) (coordinates_y(i,2)-COM_link(i,2))];
        %股関節の床反力のx方向成分の力
        ...
        %重心からCFL起始点までのベクトル
        vector4=[(coordinates_x(i,1)-COM_link(i,1)) (coordinates_y(i,1)-COM_link(i,2))];
        %CFLのx方向成分の力
        moment_CoM(i,7)=F_dist(i,7)*(-vector4(1,2));
        %CFLのy方向成分の力
        moment_CoM(i,8)=F_dist(i,8)*vector4(1,1);

        %重心からCi起始点までのベクトル
        vector5=[(coordinates_x(i,10)-COM_link(i,1)) (coordinates_y(i,10)-COM_link(i,2))];
        %CFLのx方向成分の力
        ...
        %重心からGEo起始点までのベクトル
        vector6=[(coordinates_x(i,11)-COM_link(i,1)) (coordinates_y(i,11)-COM_link(i,2))];
        %CFLのx方向成分の力
        ...
        %重心からGE停止点までのベクトル
        vector7=[(coordinates_x(i,12)-COM_link(i,1)) (coordinates_y(i,12)-COM_link(i,2))];
        %GEのx方向成分の力
        ...
        %重心からGE接点までのベクトル
        vector8=[(coordinates_x(i,8)-COM_link(i,1)) (coordinates_y(i,8)-COM_link(i,2))];
