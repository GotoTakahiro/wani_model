...
% clear;
 close all;
% clearvars
load('/Users/goto/Documents/Matlab_goto/crocodile_sim_PID-main/results/noCFLT/exp20251028noCFLT_CFL350_Ci44_CFLT100_GEo35_GE185_2.mat');
...
%リンクの運動により生じる動的な力の計算
F_hip_dyn=func_F_hip_dyn(q(:,1:10),q(:,11:20),data_accel(:,2:11),m_list,l_link_list);
...
F_hip_all=F_hip_muscletendon+F_hip_gravity+F_hip_externalforce+F_hip_hipup;%+F_hip_dyn;
...
if graph_view == true
    ...
    plot(t(:,1),F_hip_dyn(:,2),'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    ylim([-60 60]);
    % legend('Heel-$F_x$','Heel-$F_y$','Toe-$F_x$','Toe-$F_y$', 'Interpreter', 'latex', 'FontSize',20,'Location','best');
    legend('$Ci$','$CFLT$','$GEo$','$GE$','$CFL$','$dyn$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_forcehipy[4].pdf'], 'ContentType', 'vector');
    end
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_forcehipy[5].pdf'], 'ContentType', 'vector');
    end

    

    %Figure3：股関節のx方向にかかる力をそれぞれ表示1
    figure(3)
    ...
    plot(t(:,1),F_hip_dyn(:,1),'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    ylim([-100 100]);
    legend('$Ci$','$CFLT$','$GEo$','$GE$','$CFL$','$dyn$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_forcehipx[4].pdf'], 'ContentType', 'vector');
    end
    %Figure4：股関節のx方向にかかる力をそれぞれ表示2
    figure(4)
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_forcehipx[5].pdf'], 'ContentType', 'vector');
    end

    %Figure5：踵で受ける地面反力の力
    figure(5)
    plot(t(:,1),data_force(:,6),'LineWidth',2);
    hold on
    plot(t(:,1),data_force(:,7),'LineWidth',2);    
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$heel_x$','$heel_y$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_GRF_heel[1].pdf'], 'ContentType', 'vector');
    end

    %Figure6：つま先で受ける地面反力の力
    figure(6)
    plot(t(:,1),data_force(:,8),'LineWidth',2);
    hold on
    plot(t(:,1),data_force(:,9),'LineWidth',2);    
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$toe_x$','$toe_y$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_GRF_toe[1].pdf'], 'ContentType', 'vector');
    end

    %Figure7：つま先と踵の両方で受ける地面反力の力
    figure(7)
    plot(t(:,1),data_force(:,6)+data_force(:,8),'LineWidth',2);
    hold on
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_GRF[1].pdf'], 'ContentType', 'vector');
    end
