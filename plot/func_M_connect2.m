%各関節を分離したときののxy方向の接続力を計算するための関数　つま先起点
function [M_connect] = func_M_connect(q,r,l_link_list,m_list,accel_CoM,F_dist,GRF,F_all,ddphi,data_T_all,F_connect,data_accel)

    M_connect=zeros(size(q(:,1),1),29);
    ...
    for i=1:size(q(:,1),1)
        ...
        %足関節でのx,y方向の接続力
        M_connect(i,1)=Int(8)+...%J_list(4)*ddphi(i,4)+m_list(10)*(accel_CoM(i,1)*(COM_y(i,4)-coordinates_y(i,4))+accel_CoM(i,2)*(COM_x(i,4)-coordinates_x(i,4)))...
                        -(GRF(i,3)+GRF(i,5))*r-GRF(i,6)*(coordinates_x(i,5)-coordinates_x(i,4))...
                        +F_all(i,5)*r+F_met*(COM_x(i,4)-coordinates_x(i,4))-data_T_all(i,93);

        M_connect(i,5)=(GRF(i,3)+GRF(i,5))*r; %地面反力の水平方向成分
        M_connect(i,6)=GRF(i,6)*(coordinates_x(i,5)-coordinates_x(i,4)); %地面反力の垂直方向成分
        M_connect(i,7)=-F_all(i,5)*r; %F_GE方向成分
        M_connect(i,8)=-F_met*(COM_x(i,4)-coordinates_x(i,4));%重力方向成分
        M_connect(i,9)=-F_connect(i,15)*(coordinates_y(i,5)-coordinates_y(i,4));%toex成分
        M_connect(i,10)=F_connect(i,16)*(coordinates_x(i,5)-coordinates_x(i,4));%toey成分
        M_connect(i,32)=M_connect(i,5)+M_connect(i,6)+M_connect(i,7)+M_connect(i,8)+M_connect(i,9)+M_connect(i,10)+data_T_all(i,93);

        %膝関節周りのモーメントの計算
        M_connect(i,2)=M_connect(i,1)+Int(7)+...%J_list(3)*ddphi(i,3)+m_list(7)*(accel_CoM(i,3)*(COM_y(i,3)-coordinates_y(i,3))+accel_CoM(i,4)*(COM_x(i,3)-coordinates_x(i,3)))...
        ...
        M_connect(i,11)=-F_connect(i,13)*(coordinates_y(i,4)-coordinates_y(i,3)); %関節力の水平方向成分
        M_connect(i,12)=F_connect(i,14)*(coordinates_x(i,4)-coordinates_x(i,3)); %関節力の垂直方向成分
        ...
        %股関節周りのモーメントの計算
        M_connect(i,3)=M_connect(i,2)+Int(6)...%+J_list(2)*ddphi(i,2)+m_list(6)*(accel_CoM(i,7)*(COM_y(i,2)-coordinates_y(i,2))+accel_CoM(i,8)*(COM_x(i,2)-coordinates_x(i,2)))...
        ...
        M_connect(i,16)=-F_connect(i,11)*(coordinates_y(i,3)-coordinates_y(i,2)); %関節力の水平方向成分
        M_connect(i,17)=F_connect(i,12)*(coordinates_x(i,3)-coordinates_x(i,2)); %関節力の垂直方向成分
        ...
        %原点周りのモーメントの計算
        M_connect(i,4)=M_connect(i,3)+Int(5)...%+J_list(1)*ddphi(i,1)+(m_list(4)+m_list(5))*(accel_CoM(i,11)*(COM_y(i,1)-coordinates_y(i,1))+accel_CoM(i,12)*(COM_x(i,1)-coordinates_x(i,1)))...
        ...
        M_connect(i,25)=-F_connect(i,9)*(coordinates_y(i,2)-coordinates_y(i,1)); %関節力の水平方向成分
        M_connect(i,26)=F_connect(i,10)*(coordinates_x(i,2)-coordinates_x(i,1)); %関節力の垂直方向成分
