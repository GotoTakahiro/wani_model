...
load('/Users/goto/Documents/MATLAB/crocodile_sim_PID-main/plot/results/m5/exp20251028_CFL350_Ci44_CFLT100_GEo35_GE185.mat');
...
M_connect = func_M_connect2(q,r,l_link_list,m_list,accel_CoM,F_dist,GRF,data_F_all,ddphi,data_T_all,F_connect,data_accel);
%各筋腱の長さを計算
...
if graph_view == true
    ...
    xlim([0 time_lim]);
    %ylim([-120 40]);
    legend('$grav$','$toe_x$','$toe_y$','$N_x$','$N_y$','$T_{ankleex}$','$FGE$','$All$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Torque [Nm]');
    if graph_save == true
    ...
    %Figure11：関節力のx方向成分の力を表示
    figure(11)
    plot(t(:,1),F_connect(:,9),'LineWidth',2);
    hold on
    plot(t(:,1),F_connect(:,11),'LineWidth',2);
    plot(t(:,1),F_connect(:,13),'LineWidth',2);
    plot(t(:,1),F_connect(:,15),'LineWidth',2);
    %plot(t(:,1),F_dist(:,1)+F_dist(:,3)+F_dist(:,5)+F_dist(:,7),'LineWidth',2); 
    %plot(t(:,1),GRF_x,'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$hip$','$knee$','$ankle$','$toe$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_connectfx[2].pdf'], 'ContentType', 'vector');
    end

    %Figure12：関節力のx方向成分の力を表示
    figure(12)
    plot(t(:,1),F_connect(:,10),'LineWidth',2);
    hold on
    plot(t(:,1),F_connect(:,12),'LineWidth',2);
    plot(t(:,1),F_connect(:,14),'LineWidth',2);
    plot(t(:,1),F_connect(:,16),'LineWidth',2);
    %plot(t(:,1),F_dist(:,1)+F_dist(:,3)+F_dist(:,5)+F_dist(:,7),'LineWidth',2); 
    %plot(t(:,1),GRF_x,'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_connectfy[2].pdf'], 'ContentType', 'vector');
    end
