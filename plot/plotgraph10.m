...
load('/Users/goto/Documents/MATLAB/crocodile_sim/plot/results/plotall3/exp20251028_CFL350_Ci44_CFLT100_GEo35_GE185.mat');
...
if graph_view == true
    ...
    %Figure1：股関節のリンク角に対する各張力の表示
    figure(1)
    axes1 = axes('Parent',gcf);
    plot(phi(:,2),data_F_all(:,2),'LineWidth',2); %Ci
    hold on
    plot(phi(:,2),data_F_all(:,3),'LineWidth',2);
    plot(phi(:,2),data_F_all(:,4),'LineWidth',2);
    plot(phi(:,2),data_F_all(:,5),'LineWidth',2);
    hold off
    %ylim([0 time_lim]);
    xlim([-3.2 3.2]);
    ...
    legend('$Ci$','$CFLT$','$GEo$','$GE$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Angle [rad]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_a_f[1].pdf'], 'ContentType', 'vector');
    end
    
    %Figure2：膝関節のリンク角に対する各張力の表示
    figure(2)
    axes1 = axes('Parent',gcf);
    plot(phi(:,3),data_F_all(:,2),'LineWidth',2); %Ci
    hold on
    plot(phi(:,3),data_F_all(:,3),'LineWidth',2);
    plot(phi(:,3),data_F_all(:,4),'LineWidth',2);
    plot(phi(:,3),data_F_all(:,5),'LineWidth',2);
    hold off
    %ylim([0 time_lim]);
    xlim([-3.2 3.2]);
    xticklabels({'-\pi','-\pi/2','0','\pi/2','\pi'})
    ...
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_a_f[2].pdf'], 'ContentType', 'vector');
    end

    %Figure3：筋腱張力によってx方向にかかる力を表示
    figure(3)
    plot(t(:,1),F_dist(:,1),'LineWidth',2);
    hold on
    plot(t(:,1),F_dist(:,3),'LineWidth',2);
    plot(t(:,1),F_dist(:,11),'LineWidth',2);
    plot(t(:,1),F_dist(:,7),'LineWidth',2);
    plot(t(:,1),F_dist(:,1)+F_dist(:,3)+F_dist(:,11)+F_dist(:,7),'LineWidth',2); 
    %plot(t(:,1),GRF_x,'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$Ci$','$GEo$','$GE$','$CFL$','$All$','GRF','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_musclefx[1].pdf'], 'ContentType', 'vector');
    end

    %Figure4：筋腱張力によってy方向にかかる力を表示
    figure(4)
    plot(t(:,1),F_dist(:,2),'LineWidth',2);
    hold on
    plot(t(:,1),F_dist(:,4),'LineWidth',2);
    plot(t(:,1),F_dist(:,12),'LineWidth',2);
    plot(t(:,1),F_dist(:,8),'LineWidth',2);
    plot(t(:,1),F_dist(:,2)+F_dist(:,4)+F_dist(:,12)+F_dist(:,8),'LineWidth',2);    
    %plot(t(:,1),F_gravity(:,1),'LineWidth',2);
    %plot(t(:,1),GRF_y,'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$Ci$','$GEo$','$GE$','$CFL$','$All$','$gravity$','GRF','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Force [N]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_musclefy[1].pdf'], 'ContentType', 'vector');
    end

    %Figure5：筋腱張力によってy方向にかかる力を表示
    figure(5)
    plot(t(:,1),phi(:,2)-phi(:,3),'LineWidth',2);
    xlim([0 time_lim]);
    %ylim([-15 15]);
    legend('$dphi_{2-3}$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Degree [rad]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_angle[3].pdf'], 'ContentType', 'vector');
    end

    %Figure6：筋腱張力によってy方向にかかる力を表示
    figure(6)
    axes1 = axes('Parent',gcf);
    plot(t(:,1),q(:,6),'LineWidth',2);
    hold on
    plot(t(:,1),q(:,7),'LineWidth',2);
    plot(t(:,1),q(:,8),'LineWidth',2);
    hold off
    xlim([0 time_lim]);
    %ylim([-15 15]);
    yticklabels({'-\pi','-\pi/2','0','\pi/2','\pi'})
    set(axes1,'FontName','Times New Roman','FontSize',26,'YGrid','off','YTick',[-pi -pi/2 0 pi/2 pi]);
    legend('$\theta_2$','$\theta_3$','$\theta_4$','Interpreter', 'latex', 'FontSize',12,'Location','northeast');
    xlabel('Time [s]');
    ylabel('Angle [rad]');
    if graph_save == true
        exportgraphics(gca, [save_path name '_angle[3].pdf'], 'ContentType', 'vector');
